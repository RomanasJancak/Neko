...
<div class="container mt-5">
  <div class="max-w-4xl mx-auto p-6 bg-gray-900 shadow-lg rounded-lg">
    ...
    @if($editId)
        @php $editStatus = $userStatuses->firstWhere('id', $editId); @endphp
        <form action="{{ route('user-statuses.update', $editStatus) }}" method="POST" class="mb-8 flex gap-4">
            @csrf
            @method('PATCH')
            <div class="flex-1">
                <input type="text" name="name" value="{{ old('name', $editStatus->status->name ?? '') }}" placeholder="Edit Status Name" 
            ...
            <a href="{{ route('user-statuses.index') }}" class="text-gray-400 hover:text-gray-200 ml-2 self-center">Cancel</a>
        </form>
    @else
        <form action="{{ route('user-statuses.store') }}" method="POST" class="mb-8 flex gap-4">
            @csrf
            <div class="flex-1">
                <input type="text" name="name" value="{{ old('name') }}" placeholder="New Status Name (e.g. Sick, Vacation)" 
                       class="w-full border-gray-700 bg-gray-800 text-gray-100 rounded-md shadow-sm focus:border-indigo-400 focus:ring-indigo-400" required>
                @error('name') <span class="text-red-400 text-sm">{{ $message }}</span> @enderror
            </div>
            <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">
            ...
            <tr>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right flex gap-2 justify-end">
                    <form action="{{ route('user-statuses.index') }}" method="GET" style="display:inline;">
                        <input type="hidden" name="edit_id" value="{{ $us->id }}">
                        <button type="submit" class="text-indigo-400 hover:text-indigo-600">Edit</button>
                    </form>
                    <form action="{{ route('user-statuses.destroy', $us) }}" method="POST" style="display:inline;" onsubmit="return confirm('Deleting this will affect historical daily logs. Continue?')">
                        @csrf
                        @method('DELETE')
