...
<div class="container mt-5">
    <div class="row">
        <div class="col-md-12">
            <h1>Create Job</h1>
            <form method="POST" action="{{ route('job.update', $job->id) }}" class="row g-3">
                @csrf
                @method('PUT')
                <div class="form-group col-md-2">
                    ...
                    <select id="courrier_id" name="courrier_id" class="form-control" >
                        @foreach($couriers as $courier)
                            <option value="{{ $courier->id }}" {{ $job->courrier_id == $courier->id ? 'selected' : '' }}>
                                {{ $courier->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-2">
                    ...
                    <select id="status_id" name="status_id" class="form-control">
                        ...
                        <option value="{{ $status->id }}" {{ $job->status_id == $status->id ? 'selected' : '' }}>
                                {{ $status->name }}
                            </option>
                        @endforeach
                    </select>
                            ...
                            <label for="sender_id">Sender</label>
                                ...
                                <select id="sender_id" name="sender_id" class="form-control"  size="3">
                                    @foreach($clients as $client)
                                            <option value="{{ $client->id }}"{{ $job->sender_id == $client->id ? 'selected' : '' }}>
                                                {{ $client->name }}
                                            </option> 
                                    @endforeach
                                </select>
                    ...
                    <div class="row g-3 ">
                        <div class="form-group col-md">
                            <label for="collection_details">Collection Details</label>
                                <textarea id="collection_details" name="collection_details" class="form-control" placeholder="Information about delivery to courier" 
                                value="{{$job->collection_details}}">
                            </textarea>
                        </div>
                    </div>
                    <div class="row g-3 ">
                ...
                <div class="form-group">
                    ...
                    value="{{$job->receiverContacts}}"
                    >
                </div>
                <div class="row g-3 ">
                        <div class="form-group col-md">
                            <label for="dropoff_details">DropOff Details</label>
                                <textarea id="dropoff_details" name="dropoff_details" class="form-control" placeholder="Information about delivery to courier" 
                                value="{{$job->dropoff_details}}"
                                ></textarea>
                        </div>
                    </div>
                <div class="form-group">
                ...
                <a href="{{ route('job.index') }}" class="btn btn-warning">Back to Jobs</a>
            </form>
